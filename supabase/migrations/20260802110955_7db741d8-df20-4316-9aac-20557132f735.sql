...
drop policy if exists "Admins manage all folders" on public.document_folders;
create policy "Admins manage all folders" on public.document_folders
  for all to authenticated
  using (public.has_role(auth.uid(), 'admin'))
  with check (public.has_role(auth.uid(), 'admin'));

drop policy if exists "Authenticated view non-archived folders" on public.document_folders;
...
drop policy if exists "Students view assigned documents" on public.documents;
